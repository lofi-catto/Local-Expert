...
* `futago_enqueue_style`                 enqueue style
* `futago_enqueue_theme_style`           load theme-specific style (default: load)
* `futago_load_textdomain`               load theme's textdomain
* `futago_register_menus`                register the theme's default menu location
* `futago_add_theme_support`             register the various supported features
* `futago_add_woocommerce_support`       register woocommerce features, including product-gallery zoom, swipe & lightbox features
* `futago_register_elementor_locations`  register elementor settings
* `futago_content_width`                 set default content width to 800px
* `futago_page_title`                    show\hide page title (default: show)
* `futago_viewport_content`              modify `content` of `viewport` meta in header
...
* Tweak: Remove header tags with `futago_page_title` filter
...
* New: Added `futago_page_title` filter for show\hide page title
* New: Added `futago_enqueue_theme_style` filter for enqueue theme-specific style
...
= 2.0.2 - 2019-05-13 =
* Tweak: Added `futago_content_width` filter, as per WordePress best practice
...
* Tweak: Change functions.php methods names prefix from `futago_theme_` to `futago_`
...
* New: Added `futago_theme_load_textdomain` filter for load theme's textdomain
* New: Added `futago_theme_register_menus` filter for register the theme's default menu location
* New: Added `futago_theme_add_theme_support` filter for register the various supported features
* New: Added `futago_theme_add_woocommerce_support` filter for register woocommerce features, including product-gallery zoom, swipe & lightbox features
* New: Added `futago_theme_enqueue_style` filter for enqueue style
* New: Added `futago_theme_register_elementor_locations` filter for register elementor settings
